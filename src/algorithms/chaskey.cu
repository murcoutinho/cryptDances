...
#define SR1(a,b,c,d) \
  do { \
    a += b; b=ROTL(b, 5); b ^= a; a=ROTL(a,16); \
    c += d; d=ROTL(d, 8); d ^= c; \
  } while(0)
...
{
  for(int r=last_subround+1; r<=subrounds+last_subround; r++)
  {
    chaskey_subround(state, r);
  }
}
...
int test_subrounds()
{
  uint32_t state[4] = {0x45678912, 0x12345678, 0x87654321, 0x88113355};
  uint32_t alt_state[4] = {0x45678912, 0x12345678, 0x87654321, 0x88113355};

  chaskey_permute(state);
  chaskey_subrounds(alt_state, CHASKEY_ROUNDS*2, 0);
  if(memcmp(state, alt_state, 4*sizeof(uint32_t))) return 1;

  chaskey_permute(state);
  chaskey_subrounds(alt_state, 1, 0);
  chaskey_subrounds(alt_state, CHASKEY_ROUNDS*2-1, 1);
  if(memcmp(state, alt_state, 4*sizeof(uint32_t))) return 1;
...
int main() {
  ...
  if (test_subrounds()) printf("Bad subround impl");
